...
pvalue.cor6<-p.value.compute(wave.cor.list6[[4]],proc.length=dim(data)[1], sup=0.44, num.levels=6)
...
data<-data[,1:5]
n.regions<-dim(data)[2]


#Construction of the correlation matrices for each region hindered by ADHD and Depression 
wave.cor.list<-const.cor.list(data, method = "modwt" ,wf = "la8", n.levels = 6,
boundary = "periodic", p.corr = 0.975)
...
for(i in 1:nmax){
    n.sup<-sup.seq[i]
    #Construction of the adjacency matrices associated to each Region of the hindered regions
...
plot(sup.seq,(1:nmax)/2,type='n',xlab='Correlation Threshold',ylab='Global Efficiency',
cex.axis=2,cex.lab=2,xlim=c(0,1),ylim=c(0,1))
for(i in 1:n.levels){
    lines(sup.seq,Eglob[i,],type='l',col=i,lwd=2)
}
legend(x="topright",legend=c("Region 1","Region 2","Region 3","Region 4",
"Region 5","Region 6"),fill=TRUE,col=(1:n.levels),lwd=2)